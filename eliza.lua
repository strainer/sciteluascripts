...
------------------------------------------------------------------------
function ElizaHandler(c)
--~ if not buffer[SIG] then return end    -- verify buffer signature
  if c == "\n" then                     -- user typed [Enter]
	  pane=get_pane()
		if pane==editor then return end
		
    local lnUser = output:LineFromPosition(editor.CurrentPos) - 1
    if lnUser < 0 then return end
    -- grab line without ending newlines
    local lbeg = output:PositionFromLine(lnUser)
    local lend = output.LineEndPosition[lnUser]
    local text = output:textrange(lbeg, lend)
    -- perform Eliza text processing
    if text =="ok" then
			scite_OnChar(ElizaHandler,'remove')
			return true
		elseif text ~= "" then
      output:AddText(Eliza(text))
    end
  end
  return true
end
...
function eliza()
  ...
  introg9=[[
WELCOME TO ANOTHER SESSION WITH YOUR COMPUTER PSYCHIATRIST, ELIZA
