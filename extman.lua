...
function get_line(pane,lineno)
	...
	if not lineno then
		...
		lineno = pane:LineFromPosition(line_pos)
	end
...
if GTK then
	...
	--return 0==os.execute('dir /AD /W "'..path..'"')
	--os.execute causes comwin to flash
	function scite_DirectoryExists(path)
		return true
	end
end
...
function reload_script()
	...
	local prmpt=output:GetLine(output.LineCount-1)
	if not prmpt then prmpt="x" end
	print('\n+ Lua dofile ( '..current_file.." )")
	if string.find(prmpt,";") then trace(prmpt)
	else
		tracePrompt()
	end
...
--~ remdebug.engine.start()  dont know about this yet
...
--additions by a strain

sanker={} --selection anchors, a table entry for each buffer of last selected anchor
scurren={} --selection currentPoss..
trans={} --table for placing transient test / utility functions in global scope
autocnames= {}
openedbuffers= {}
		 
function get_word(pane)
	local wd=pane:textrange(wordstart(pane),wordend(pane))
	return string.gsub(wd, "\t", "")
end

function wordstart(pane)
	local l=pane:LineFromPosition(pane.CurrentPos)
	local s=pane:PositionFromLine(l)
	local r=pane:WordStartPosition(pane.CurrentPos)
	if r<s then r=s end
	return r
end

function wordend(pane)
	local l=pane:LineFromPosition(pane.CurrentPos)
	local s=pane.LineEndPosition[l]
	local r=pane:WordEndPosition(pane.CurrentPos)
	if r>s then r=s end
	return r
end
...
--------------------
--good function from ahkguy?
--used by textstep and autocplete ...

function shouldIgnorePos(pos,pane,lex)
 
	local styletype = { -- comment or string styles styles
		[200] = {[1]=true,[2]=true,[6]=true,[20]=true},
		[SCLEX_LUA]  = {[1]=true,[2]=true,[3]=true,[8]=true}, -- comments, strings or error
		[SCLEX_PYTHON] = {[1]=true,[3]=true,[12]=true},
		[SCLEX_CPP] = {[1]=true,[2]=true,[3]=true,[6]=true,
			[9]=true,[14]=true,[15]=true},
		[SCLEX_HTML] = {[42]=true,[43]=true,[44]=true,[57]=true,
			[58]=true,[59]=true,[124]=true,[125]=true},
		[SCLEX_PROPERTIES] = {[1]=true} 
	}
	return styletype[lex] and styletype[lex][ pane.StyleAt[pos] ]
end 

--used by textstep and autocplete ...
function isquoted(pos,pane,lex)
	local styletype = { -- comment or string styles styles
		[200] = {[1]=true,[2]=true,[6]=true,[20]=true}, --SCLEX_AHK
		[SCLEX_LUA]  = {[6]=true,[7]=true,[8]=true}, -- strings
		[SCLEX_PYTHON] = {[3]=true,[4]=true},
		[SCLEX_PYTHON2] = {[3]=true,[4]=true},
		[SCLEX_CPP] = {[6]=true,[7]=true },
		[SCLEX_CSS] = {[13]=true,[14]=true },
		[SCLEX_CONF] = {[6]=true },
		[SCLEX_CMAKE] = {[2]=true,[3]=true },
		[SCLEX_ASM] = {[3]=true },
		[SCLEX_RUBY] = {[6]=true },
		[SCLEX_HTML] 
		={[119]=true,[120]=true,[6]=true,[7]=true,[24]=true,
			[25]=true,[48]=true,[49]=true,[63]=true,[64]=true,
			[75]=true,[85]=true,[94]=true,[95]=true,[109]=true,[110]=true}
	}
		
	return styletype[lex] and styletype[lex][ pane.StyleAt[pos] ]
end

function isInTable(tablo,thing)
	for v=1,table.getn(tablo) do
		--trace(v.."v"..thing.."t")  --ooo  ooo  thing
		if tablo[v]==thing then return true end
	end
	return false
end

-------
function is_white(c) --unused?
	return nil==string.find(c, "[^\t\r\n ]")
end
-------------------

Slide={}
--~ scite_dofile('inactive.lua')
--~ scite_dofile('tcrypt.lua')
scite_dofile('initglob.lua')
scite_dofile('watcher.lua')
scite_dofile('autocom.lua')
scite_dofile('recoin.lua')
scite_dofile('carecrow.lua')
scite_dofile('propcalled.lua')
scite_dofile('zlot1.lua')
scite_dofile('textstep.lua')
scite_dofile('blockselector.lua')
scite_dofile('withattr.lua')
scite_dofile('codepaste.lua')
scite_dofile('sort.lua')  --sorts lines of text
...
morkedfiles={}
scite_dofile('arcfour.lua')
scite_dofile('fcrypt.lua')

--scite_dofile('fruiter.lua')
--scite_dofile('test.lua')
