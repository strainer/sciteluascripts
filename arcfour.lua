...
local function generate(self, count, dump)
	...
	self.i, self.j = i, j
	if not dump then return table.concat(o) end
end
...
local function scram(self, plaintext, peep)
	
	local jitt=(#plaintext*7)%31
	while (#peep-jitt)<#plaintext do peep=peep..peep end
	local r = { }
	local byte = string.byte
	local char = string.char
	
	for i = 1, #plaintext do
		r[i] = char(byte_xor[byte(plaintext, i)][byte(peep, (i+jitt) )])
	end
...
local function Sstash(self,dum)
	self.Sstashs = tcopy(self.S)
	self.istash, self.jstash = self.i, self.j
	return
end

local function SstashU(self,dum)
	self.S = tcopy(self.Sstashs)
	self.i, self.j = self.istash, self.jstash
	return
end
--***

function new(key)
	local Sstashs,istash,jstash
	local S = { }
	local r = {
		S = S, i = 0, j = 0,
		istash=istash,jstash=jstash,
		generate = generate,
		Sstash = Sstash,
		SstashU = SstashU,
		cipher = cipher,
